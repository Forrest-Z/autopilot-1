...
Box2d Node3d::GetBoundingBox(const VehicleParam& vehicle_param_,
  ...
  double ego_length = vehicle_param_.vehicle_length;
  double ego_width = vehicle_param_.vehicle_width;
  double shift_distance =
      ego_length / 2.0 - vehicle_param_.vehicle_back_edge_to_center;
  Box2d ego_box(
...
std::string Node3d::ComputeStringIndex(int x_grid, int y_grid, int phi_grid) {
     std::string str = std::to_string(x_grid) + "_";
     str = std::move(str) + std::to_string(y_grid);
     str = std::move(str) + "_";
    str = std::move(str) + std::to_string(phi_grid);
    return str;
