...
std::string AABox2d::DebugString() const {
  std::string str = "aabox2d ( center = " + center_.DebugString();
  str = std::move(str) +  "  length = ";
  str = std::move(str) + std::to_string(length_);
  str = std::move(str) + "  width = ";
  str = std::move(str) + std::to_string(width_);
  return str;
