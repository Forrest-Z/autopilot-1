...
{
	return map_projection_init_timestamped(ref, lat_0, lon_0, user_time::get_millis());
}
...
int map_projection_project(const struct map_projection_reference_s *ref, double lat, double lon, float *x, float *y)
{
	...
	const double arg = math::Clamp(ref->sin_lat * sin_lat + ref->cos_lat * cos_lat * cos_d_lon, -1.0,  1.0);
					...
					double *lat_target, double *lon_target)
{
	if (fabsf(dist) < FLT_EPSILON) {
		...
		*lon_target = lon_A;
		...
		float heading = get_bearing_to_next_waypoint(lat_A, lon_A, lat_B, lon_B);
		heading = math::wrap_2PI(heading + M_PI);
		waypoint_from_heading_and_distance(lat_A, lon_A, heading, dist, lat_target, lon_target);
	}
}
			 ...
			 double lat_start, double lon_start, double lat_end, double lon_end)
{
	...
	float bearing_diff = math::wrap_PI(bearing_track - bearing_end);

	// Return past_end = true if past end point of line
	if (bearing_diff > M_PI_2 || bearing_diff < -M_PI_2) {
	...
	if (sinf(bearing_diff) >= 0) {
		crosstrack_error->bearing = math::wrap_PI(bearing_track - M_PI_2);
			...
			float radius, float arc_start_bearing, float arc_sweep)
{
	...
	if (arc_sweep >= 0.0f) {
		...
		if (bearing_sector_end > 2.0f * M_PI) { bearing_sector_end -= (2 * M_PI); }

	} else {
		bearing_sector_end = arc_start_bearing;
		bearing_sector_start = arc_start_bearing - arc_sweep;

		if (bearing_sector_start < 0.0f) { bearing_sector_start += (2 * M_PI); }
	}
	...
	if (in_sector) {
		...
		if (dist_to_center <= radius) {
			...
			crosstrack_error->bearing = bearing_now + M_PI;
		...
		double end_disp_x = (double)radius * sin((double)math::wrap_PI(arc_start_bearing + arc_sweep));
		double end_disp_y = (double)radius * cos((double)math::wrap_PI(arc_start_bearing + arc_sweep));
	...
	crosstrack_error->bearing = math::wrap_PI(crosstrack_error->bearing);
	return_value = 0;
...
void intchangle_double_to_string(double a, uint8_t &deg, uint8_t &min, uint8_t &sec, uint8_t &msec)
{
	char strchar[20] = {0};
	...
	sprintf(strchar, "%f", (a - deg) * 60);
	min = atoi(strchar);

	sprintf(strchar, "%.0f", ((a - deg) * 60 - min)*60*100);
	sec = atoi(strchar) / 100;
	msec = atoi(strchar) % 100;
}
