...
std::pair<double, double> ReedShepp::calc_tau_omega(const double u,
                                                    ...
                                                    const double eta,
                                                    const double phi) {
  double delta = math::NormalizeAngle(u - v);
  ...
  double tau = 0.0;
  if (t2 < 0) {
    tau = math::NormalizeAngle(t1 + M_PI);
  } else {
    tau = math::NormalizeAngle(t1);
  }
  double omega = math::NormalizeAngle(tau - u + v - phi);
...
bool ReedShepp::ShortestRSP(const std::shared_ptr<Node3d> start_node,
  ...
  if (!GenerateRSPs(start_node, end_node, &all_possible_paths)) {
    LOG(INFO) << "Fail to generate different combination of Reed Shepp "
              "paths";
    return false;
  }
  ...
  if (!GenerateLocalConfigurations(start_node, end_node,
    ...
    LOG(INFO) << "Fail to generate local configurations(x, y, phi) in SetRSP";
    return false;
  }
  ...
  if (std::abs(all_possible_paths[optimal_path_index].x.back() -
    ...
    LOG(INFO) << "end x, y, phi are: " << end_node->GetX() << ", "
...
bool ReedShepp::GenerateRSPs(const std::shared_ptr<Node3d> start_node,
  ...
  if (FLAGS_enable_parallel_hybrid_a) {
    // LOG(INFO)  << "parallel hybrid a*";
    if (!GenerateRSPPar(start_node, end_node, all_possible_paths)) {
      LOG(INFO)  << "Fail to generate general profile of different RSPs";
      return false;
    }
  } else {
...
bool ReedShepp::SCS(const double x, const double y, const double phi,
  ...
  if (SLS_param.flag &&
    ...
    LOG(INFO)  << "Fail at SetRSP with SLS_param";
    return false;
  }
  ...
  if (SRS_param.flag &&
    ...
    LOG(INFO)  << "Fail at SetRSP with SRS_param";
    return false;
  }
  return true;
}
...
bool ReedShepp::CSC(const double x, const double y, const double phi,
  ...
  if (LSL1_param.flag &&
    ...
    LOG(INFO)  << "Fail at SetRSP with LSL_param";
    return false;
  }
  ...
  if (LSL2_param.flag &&
    ...
    LOG(INFO)  << "Fail at SetRSP with LSL2_param";
    return false;
  }
  ...
  if (LSL3_param.flag &&
    ...
    LOG(INFO)  << "Fail at SetRSP with LSL3_param";
    return false;
  }
  ...
  if (LSL4_param.flag &&
    ...
    LOG(INFO)  << "Fail at SetRSP with LSL4_param";
    return false;
  }
  ...
  if (LSR1_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LSR1_param";
    return false;
  }
  ...
  if (LSR2_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LSR2_param";
    return false;
  }
  ...
  if (LSR3_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LSR3_param";
    return false;
  }
  ...
  if (LSR4_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LSR4_param";
    return false;
  }
  return true;
}
...
bool ReedShepp::CCC(const double x, const double y, const double phi,
  ...
  if (LRL1_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRL_param";
    return false;
  }
  ...
  if (LRL2_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRL2_param";
    return false;
  }
  ...
  if (LRL3_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRL3_param";
    return false;
  }
  ...
  if (LRL4_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRL4_param";
    return false;
  }
  ...
  if (LRL5_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRL5_param";
    return false;
  }
  ...
  if (LRL6_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRL6_param";
    return false;
  }
  ...
  if (LRL7_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRL7_param";
    return false;
  }
  ...
  if (LRL8_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRL8_param";
    return false;
  }
  return true;
}
...
bool ReedShepp::CCCC(const double x, const double y, const double phi,
  ...
  if (LRLRn1_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRLRn_param";
    return false;
  }
  ...
  if (LRLRn2_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRLRn2_param";
    return false;
  }
  ...
  if (LRLRn3_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRLRn3_param";
    return false;
  }
  ...
  if (LRLRn4_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRLRn4_param";
    return false;
  }
  ...
  if (LRLRp1_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRLRp1_param";
    return false;
  }
  ...
  if (LRLRp2_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRLRp2_param";
    return false;
  }
  ...
  if (LRLRp3_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRLRp3_param";
    return false;
  }
  ...
  if (LRLRp4_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRLRp4_param";
    return false;
  }
  return true;
}
...
bool ReedShepp::CCSC(const double x, const double y, const double phi,
  ...
  if (LRSL1_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSL1_param";
    return false;
  }
  ...
  if (LRSL2_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSL2_param";
    return false;
  }
  ...
  if (LRSL3_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSL3_param";
    return false;
  }
  ...
  if (LRSL4_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSL4_param";
    return false;
  }
  ...
  if (LRSR1_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSR1_param";
    return false;
  }
  ...
  if (LRSR2_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSR2_param";
    return false;
  }
  ...
  if (LRSR3_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSR3_param";
    return false;
  }
  ...
  if (LRSR4_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSR4_param";
    return false;
  }
  ...
  if (LRSL6_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSL6_param";
    return false;
  }
  ...
  if (LRSL7_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSL7_param";
    return false;
  }
  ...
  if (LRSL8_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSL8_param";
    return false;
  }
  ...
  if (LRSR5_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSR5_param";
    return false;
  }
  ...
  if (LRSR6_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSR6_param";
    return false;
  }
  ...
  if (LRSR7_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSR7_param";
    return false;
  }
  ...
  if (LRSR8_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSR8_param";
    return false;
  }
  return true;
}
...
bool ReedShepp::CCSCC(const double x, const double y, const double phi,
  ...
  if (LRSLR1_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSLR1_param";
    return false;
  }
  ...
  if (LRSLR2_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSLR2_param";
    return false;
  }
  ...
  if (LRSLR3_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSLR3_param";
    return false;
  }
  ...
  if (LRSLR4_param.flag &&
    ...
    LOG(INFO) << "Fail at SetRSP with LRSLR4_param";
    return false;
  }
  return true;
}
...
void ReedShepp::LSL(const double x, const double y, const double phi,
  ...
  std::pair<double, double> polar =
      math::Cartesian2Polar(x - std::sin(phi), y - 1.0 + std::cos(phi));
  double u = polar.first;
  double t = polar.second;
  double v = 0.0;
  if (t >= 0.0) {
    v = math::NormalizeAngle(phi - t);
    if (v >= 0.0) {
      param->flag = true;
      param->u = u;
      param->t = t;
...
void ReedShepp::LSR(const double x, const double y, const double phi,
  ...
  if (u1 >= 4.0) {
    ...
    t = math::NormalizeAngle(t1 + theta);
    v = math::NormalizeAngle(t - phi);
    if (t >= 0.0 && v >= 0.0) {
      param->flag = true;
      param->u = u;
      param->t = t;
...
void ReedShepp::LRL(const double x, const double y, const double phi,
  ...
  if (u1 <= 4.0) {
    ...
    t = math::NormalizeAngle(t1 + 0.5 * u + M_PI);
    v = math::NormalizeAngle(phi - t + u);
    if (t >= 0.0 && u <= 0.0) {
      param->flag = true;
      param->u = u;
      param->t = t;
...
void ReedShepp::SLS(const double x, const double y, const double phi,
                    RSPParam* param) {
  double phi_mod = math::NormalizeAngle(phi);
  double xd = 0.0;
  double u = 0.0;
  double t = 0.0;
  double v = 0.0;
...
void ReedShepp::LRSR(const double x, const double y, const double phi,
  ...
  double xi = x + std::sin(phi);
  double eta = y - 1.0 - std::cos(phi);
  std::pair<double, double> polar = math::Cartesian2Polar(-eta, xi);
  ...
  if (rho >= 2.0) {
    t = theta;
    u = 2.0 - rho;
    v = math::NormalizeAngle(t + 0.5 * M_PI - phi);
...
void ReedShepp::LRSL(const double x, const double y, const double phi,
  ...
  double xi = x - std::sin(phi);
  double eta = y - 1.0 + std::cos(phi);
  std::pair<double, double> polar = math::Cartesian2Polar(xi, eta);
  double rho = polar.first;
  double theta = polar.second;
  double r = 0.0;
  double t = 0.0;
  ...
  if (rho >= 2.0) {
    ...
    u = 2.0 - r;
    t = math::NormalizeAngle(theta + std::atan2(r, -2.0));
    v = math::NormalizeAngle(phi - 0.5 * M_PI - t);
    if (t >= 0.0 && u <= 0.0 && v <= 0.0) {
      param->flag = true;
      param->u = u;
      param->t = t;
...
bool ReedShepp::SetRSP(const int size, const double* lengths, const char* types,
  ...
  if (path.total_length <= 0.0) {
    LOG(ERROR) << "total length smaller than 0";
    return false;
  }
...
bool ReedShepp::GenerateLocalConfigurations(
  ...
  for (size_t i = 0; i < px.size(); ++i) {
    ...
    shortest_path->phi.push_back(
        math::NormalizeAngle(pphi.at(i) + start_node->GetPhi()));
  }
...
#pragma omp parallel for schedule(dynamic, 2) num_threads(8)
  for (int i = 0; i < RSP_nums; ++i) {
    ...
    if (tmp_length > 0) {
      if (RSP_param.flag &&
        ...
        LOG(ERROR) << "Fail at SetRSP, idx#: " << i;
        succ = false;
      }
    }
  }
  ...
  if (!succ) {
    LOG(ERROR) << "RSP parallel fails";
    return false;
  }
  if (all_possible_paths->size() == 0) {
    LOG(ERROR) << "No path generated by certain two configurations";
    return false;
  }
  return true;
}
