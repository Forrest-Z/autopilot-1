...
static void initializeDeviceProp() {
  if (!m_devicePropInitialized) {
    ...
    if (first) {
      ...
      int num_devices;
      cudLOG(ERROR)_t status = cudaGetDeviceCount(&num_devices);
 ...
 public:
  ...
  CudaStreamDevice(const cudaStream_t* stream, int device = -1)
    ...
    if (device < 0) {
      ...
      int num_devices;
      cudLOG(ERROR)_t err = cudaGetDeviceCount(&num_devices);
  ...
  virtual void* allocate(size_t num_bytes) const {
    cudLOG(ERROR)_t err = cudaSetDevice(device_);
    EIGEN_UNUSED_VARIABLE(err)
    assert(err == cudaSuccess);
    void* result;
  ...
  virtual unsigned int* semaphore() const {
    if (semaphore_ == NULL) {
      ...
      cudLOG(ERROR)_t err = cudaMemsetAsync(semaphore_, 0, sizeof(unsigned int), *stream_);
      EIGEN_UNUSED_VARIABLE(err)
      assert(err == cudaSuccess);
    }
    return semaphore_;
...
struct GpuDevice {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void memcpy(void* dst, const void* src, size_t n) const {
    ...
    cudLOG(ERROR)_t err = cudaMemcpyAsync(dst, src, n, cudaMemcpyDeviceToDevice,
                                      stream_->stream());
    EIGEN_UNUSED_VARIABLE(err)
    assert(err == cudaSuccess);
#else
...
#ifndef __CUDA_ARCH__
    cudLOG(ERROR)_t err = cudaMemsetAsync(buffer, c, n, stream_->stream());
    EIGEN_UNUSED_VARIABLE(err)
    assert(err == cudaSuccess);
#else
  eigen_assert(false && "The default device should be used instead to generate kernel code");
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void synchronize() const {
    ...
    cudLOG(ERROR)_t err = cudaStreamSynchronize(stream_->stream());
    if (err != cudaSuccess) {
...
#ifdef __CUDACC__
    cudLOG(ERROR)_t error = cudaStreamQuery(stream_->stream());
    return (error == cudaSuccess) || (error == cudLOG(ERROR)NotReady);
#else
    return false;
#endif
  }
...
#ifndef __CUDA_ARCH__
  cudLOG(ERROR)_t status = cudaDeviceSetSharedMemConfig(config);
