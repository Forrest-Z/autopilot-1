...
{   
    const uint8_t sector = math::wrap_360(yaw + (PROXIMITY_SECTOR_WIDTH_DEG * 0.5f)) / 45.0f;
    const float pitch_limited = math::Clamp(pitch, -75.0f, 74.9f);
...
void AP_Proximity_Boundary_3D::update_boundary(const Face &face)
{
    ...
    if (_distance_valid[layer][sector] && _distance_valid[layer][next_sector]) {
        shortest_distance = std::fmin(_filtered_distance[layer][sector].get(), _filtered_distance[layer][next_sector].get());
    ...
    if (_distance_valid[layer][prev_sector] && _distance_valid[layer][sector]) {
        shortest_distance = std::fmin(_filtered_distance[layer][prev_sector].get(), _filtered_distance[layer][sector].get());
