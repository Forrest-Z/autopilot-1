...
double SqrtController(double error, double p, double second_ord_lim,double dt )
{
	 double correction_rate;
    if (second_ord_lim<0.0 ||second_ord_lim == 0.0) {
        // second order limit is zero or negative.
        correction_rate = error * p;
    } else if (p== 0.0) {
        // P term is zero but we have a second order limit.
        if (error > 0.0) {
            correction_rate = sqrt(2.0f * second_ord_lim * (error));
        } else if (error < 0.0) {
            correction_rate = -sqrt(2.0f * second_ord_lim * (-error));
        } else {
        ...
        double linear_dist = second_ord_lim / Sqr(p);
        if (error > linear_dist) {
            correction_rate = sqrt(2.0f * second_ord_lim * (error - (linear_dist / 2.0f)));
    ...
    if (dt!= 0) {
        ...
        return Clamp(correction_rate, -fabs(error) / dt, fabs(error) / dt);
    } else {
